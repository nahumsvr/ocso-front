...
import { registerManager } from "@/actions/users/registerManager";

export default function CreateManagerUser({ employeeId }: { employeeId: string }) {
    const [password, setPassword] = useState<string>("");
    const registerUserById = registerManager.bind(null, employeeId, password);
    ...
    return (
        <form className="flex flex-col gap-2" action={registerUserById}>
            <TextField name="userEmail" type="text" isRequired>
...
function PasswordWithToggle({ password, setPassword }: { password: string, setPassword: (password: string) => void }) {
    ...
    return (
        <TextField className="w-full" name="userPassword" isRequired>
